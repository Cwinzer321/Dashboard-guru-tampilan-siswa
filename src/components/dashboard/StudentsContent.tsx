...
export default function StudentsContent({ students, darkMode, onAddStudent }: StudentsContentProps) {
    ...
    <div className="space-y-6">
      ...
      <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow p-6`}>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="Masukkan nama atau NIS siswa..."
                             className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 ${darkMode ? 'bg-gray-700 border-gray-600 text-gray-100 placeholder-gray-300' : 'bg-white border-gray-300 text-gray-900 placeholder-gray-500'}`}
            />
          </div>
          <div>
            <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Kelas</label>
            <select className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 ${darkMode ? 'bg-gray-700 border-gray-600 text-gray-100' : 'bg-white border-gray-300 text-gray-900'}`}>
          ...
          <div>
            <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Jenis Kelamin</label>
            <select className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 ${darkMode ? 'bg-gray-700 border-gray-600 text-gray-100' : 'bg-white border-gray-300 text-gray-900'}`}>
              <option value="" className={`${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Semua</option>
              <option value="L" className={`${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Laki-laki</option>
              <option value="P" className={`${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Perempuan</option>
            </select>
          </div>
          <div>
            <label className={`block text-sm font-medium mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Status Tes</label>
            <select className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-green-500 focus:border-green-500 ${darkMode ? 'bg-gray-700 border-gray-600 text-gray-100' : 'bg-white border-gray-300 text-gray-900'}`}>
              <option value="" className={`${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Semua</option>
              <option value="completed" className={`${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Sudah Tes</option>
              <option value="pending" className={`${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Belum Tes</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow`}>
        <div className={`px-6 py-4 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <h3 className={`text-lg font-semibold ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Daftar Siswa ({students.length})</h3>
        </div>
          ...
          <table className={`min-w-full divide-y ${darkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
            <thead className={darkMode ? 'bg-gray-700' : 'bg-gray-50'}>
              <tr>
                ...
                <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Jenis Kelamin</th>
                <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Status Tes</th>
      ...
      <DeleteConfirmationModal
        ...
        itemName={selectedStudent ? `${selectedStudent.name} (${selectedStudent.nis})` : undefined}
        darkMode={darkMode}
      />
